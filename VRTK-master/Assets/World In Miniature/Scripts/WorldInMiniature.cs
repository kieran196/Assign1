...
public class WorldInMiniature : MonoBehaviour {
    ...
    private List<string> ignorableObjectsString = new List<string>{ "[CameraRig]", "Directional Light", "background", "roof"};
    public List<GameObject> ignorableObjects = new List<GameObject>();
    private float scaleAmount = 60f;
    ...
    void createWiM() {
        if (controller.GetPressDown(SteamVR_Controller.ButtonMask.ApplicationMenu)) {
            if (WiMAactive == false) {
                vrtkScripts.SetActive(false);
                WiMAactive = true;
                print("Create world clone");
                for (int i = 0; i < allSceneObjects.Length; i++) {
                    print(allSceneObjects[i].name);
                    if (!ignorableObjectsString.Contains(allSceneObjects[i].name) && !ignorableObjects.Contains(allSceneObjects[i])) {
